...
export default function (image, handles, handleKey, mmStep, mmLimit = 1) {
  ...
  if (mmStep < 0) {
    ...
    const lineAB = {
      start: Object.assign({}, ...['x', 'y'].map((key) => ({ [key]: handleA[key] }))),
      end: Object.assign({}, ...['x', 'y'].map((key) => ({ [key]: handleB[key] })))
    };

    // Create the line segment for the perpendicular arm
    const lineCD = {
      start: Object.assign({}, ...['x', 'y'].map((key) => ({ [key]: handleC[key] }))),
      end: Object.assign({}, ...['x', 'y'].map((key) => ({ [key]: handleD[key] })))
    };

    // Get the intersection point between the arms
    const intersection = external.cornerstoneMath.lineSegment.intersectLine(lineAB, lineCD);

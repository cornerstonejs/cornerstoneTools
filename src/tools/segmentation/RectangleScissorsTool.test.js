...
function setMocksAndSpies(store) {
  ...
  // Spies for Mixins
  _startOutliningRegion_Spy = jest.spyOn(mixins.rectangleSegmentationMixin, '_startOutliningRegion');
  _setHandlesAndUpdate_Spy = jest.spyOn(mixins.rectangleSegmentationMixin, '_setHandlesAndUpdate');
  _applyStrategy_Spy = jest.spyOn(mixins.rectangleSegmentationMixin, '_applyStrategy');
}
  ...
  describe('Events and Callbacks', () => {
    ...
    describe('Touch Events setup', () => {
      it('Calls right method on touchDragCallback ', () => {
        ...
        expect(mixins.rectangleSegmentationMixin._setHandlesAndUpdate).toHaveBeenCalled();
      });

      it('Calls right method on postTouchStartCallback ', () => {
        instantiatedTool.postTouchStartCallback(mockEvt);
        expect(mixins.rectangleSegmentationMixin._startOutliningRegion).toHaveBeenCalled();
      });

      it('Calls right method on touchEndCallback ', () => {
        instantiatedTool.touchEndCallback(mockEvt);
        expect(mixins.rectangleSegmentationMixin._applyStrategy).toHaveBeenCalled();
      });
    });

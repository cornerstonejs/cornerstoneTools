...
import mergeToolStateToGlobalToolState from './mergeToolStateToGlobalToolState.js';
...
describe('mergeToolStateToGlobalToolState.js', () => {
  beforeEach(() => {});

  describe('mergeToolStateToGlobalToolState', () => {
    it('should invalidate the whole toolState for any imageIds in the stack.', () => {
      const newToolState = {
        ...
        someCompletelyDifferentImageId: {
          Length: {
            data: [
              {
                // some toolData.
      ...
      const toolState = mergeToolStateToGlobalToolState(newToolState);

      console.log(toolState);

      console.log(toolState['example://1'].Length.data);

      expect(toolState['example://1'].Length.data.length).toEqual(2);

      expect(
        toolState['someCompletelyDifferentImageId'].Length.data.length
      ).toEqual(1);
    });
  });
});
